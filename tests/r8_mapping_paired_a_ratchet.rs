...
//!
//! Reads `DROIDSAW_R8_MAPPED_A_PATH` — a directory containing
...
/// corpus A.
///
/// Each entry is `(mapping_key_class, helper_method_or_wildcard,
/// justification)`. A `*` in the method slot means any method on
/// the class qualifies as an allowlisted FP — used for D8 desugar
/// host classes where every method is a desugar bridge, not a real
/// outline. Non-wildcard entries match the (class, method) pair
/// exactly.
///
/// Any new mapping-disagreement (recogniser fires, mapping says NOT
/// outlined) that isn't in this allowlist makes the ratchet panic.
/// Adding a new entry requires a justification that names the
/// structural shape (horizontal-merge bridge, D8 desugar artefact,
/// etc.) — the goal is documenting WHY the recogniser fires on a
/// non-outline, not silencing the signal.
const FP_ALLOWLIST: &[(&str, &str, &str)] = &[
    (
        "h1",
        "r",
        "horizontal-merge bridge — synthesized but NOT outline-annotated; structurally indistinguishable from a real outline at the bytecode level (≥ 20 distinct callers, single-BB body satisfying I3-I13). A known false positive from outline detection heuristics.",
    ),
    (
        "j$.time",
        "b",
        "D8 desugar — java.time backport bridge. Synthesized by D8 (not R8) so the outline annotation is absent by construction; the recogniser sees outline-shaped bytecode because the desugar bridge happens to share the structural shape.",
    ),
    (
        "j$.util.DesugarArrays",
        "*",
        "D8 desugar — java.util.Arrays utility backport. Every method on this class is a desugar bridge, not a real outline — wildcard match because future D8 releases may add methods.",
    ),
];
...
fn corpus_dir() -> Option<PathBuf> {
    let raw = std::env::var_os("DROIDSAW_R8_MAPPED_A_PATH")?;
...
#[test]
fn r8_mapping_paired_a_baseline() {
    let handle = std::thread::Builder::new()
        .name("r8_mapping_paired_a_worker".into())
...
fn baseline_main() {
    let Some(dir) = corpus_dir() else {
        eprintln!(
            "SKIP: DROIDSAW_R8_MAPPED_A_PATH unset or does not point at a directory. \
             To exercise the mapping-paired corpus A ratchet, prepare a directory with \
             R8 mapping pairs (classes*.dex + mapping.txt files), then set \
             DROIDSAW_R8_MAPPED_A_PATH=/path/to/dir."
        );
        return;
    };
    ...
    // analyst triage tracks distinct (class, method) tuples, not
    // marker occurrences. Non-empty at the end of the walk fails
    // the ratchet — any new mapping-disagreement must be triaged
    // and either added to the allowlist with justification or fixed
    // in the recogniser.
    ...
    eprintln!(
        "MAPPING-PAIRED CORPUS A BASELINE: {} DEXes ({} skipped), \
    ...
    // Mapping-mismatch gate. Any FP NOT in the pinned allowlist
    // fails the ratchet hard. The 3 known FPs (h1.r horizontal-merge
    // bridge, j$.time.b D8 desugar, j$.util.DesugarArrays.* D8
    // desugar) are covered by FP_ALLOWLIST with justifications;
    // anything else needs triage before the ratchet goes green.
